...
#' objects, it is recommended to import it into the GEE assets. See
...
#' it checks every ring, or revert them if necessary, to counter clockwise for outer rings,
#' and clockwise for inner (hole) ones. By default this is FALSE because
#' it involves large computational demands.
#'
#' @examples
#' library(rgee)
#' library(sf)
...
sf_as_ee.sfg <- function(x, check_ring_dir = FALSE) {
  # message("EPSG:4326 is assigned as a coordinate reference system")
  x <- st_sfc(x, check_ring_dir = check_ring_dir)
  oauth_func_path <- system.file("python/sf_as_ee.py", package = "rgee")
  sf_as_ee <- ee_source_python(oauth_func_path)
  geojson_list <- geojson_json(x)
