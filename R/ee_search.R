...
#' @param provider Character. Name of the dataset provided. See
...
#' and rgee::ee_search_title. 'AND' concatenates tags in
#' search and 'OR' exclude any.
...
ee_dataset <- function(quiet = FALSE, upgrade = FALSE) {
  ...
  if (file.exists(ee_dataset_file) & !upgrade) {
    ...
    if (!quiet) {
      cat("Downloading(Upgrading) Earth Engine catalog ... please wait\n")
    }
...
ee_search_type <- function(ee_dataset, type) {
  ...
  if (type %in% unique(ee_dataset_type)) {
    ...
    stop("typed argument is not valid")
  }
}
...
ee_search_provider <- function(ee_dataset, provider) {
  if (provider %in% unique(ee_dataset$provider)) {
    ...
    stop("provided argument is not valid")
  }
}
